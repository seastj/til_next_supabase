...
- `/src/apis 폴더` 생성
- 참조 : 경로 주의
- 참조 : `/src/app/api` 폴더에 작성하면 Next 가 작동된다. (금지)
...
- 현재 글이 등록 실패되었는지 `error 개발자가 코딩으로 관리한다.`
- 현재 글을 등록 오류 발생시 `error 개발자가 코딩으로 관리한다.`
