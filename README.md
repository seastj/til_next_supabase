...
export async function uploadsImage({ filePath, file }: ImageType) {
  // 파일을 업로드 함.
...
type OpenState = {
  title: "삭제 확인";
  discription: "삭제하시면 내용이 모두 제거됩니다.";
  onPositive?: () => consol.log("확인 클릭");
  onNegative?: () => consol.log("취소 클릭");
};
```
